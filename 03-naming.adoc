...
Comme chacun le sait, après le choix entre espace et tabulation, un nommage correcte reste souvent un des meilleurs moyen d'avoir du code lisible.

Or à priori les merveilleuses lambda nous on fait perdre nos noms !
...
myList.forEach(System.out::println); // équivaut à
myList.forEach(element -> System.out.println(element));
----
