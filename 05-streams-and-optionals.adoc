...
Elles ne déclenchent rien et ne font que configurer une future utilisation.
...
----
- `map` : Prend en paramètre une `Function` et l'applique sur chacun des éléments du stream.
...
- `peek` : Il permet de "jeter un coup d'oeil" sur les éléments du stream, sans les transformer, et sans être final non plus. Dans la grande majorité des cas, si vous apercevez un `peek`, fuyez.
...
==== Rechercher
Les méthodes de recherche d'éléments renvoient des `Optional<T>`. Un `Optional` encapsule un objet java (ou du vide) et fournit une api qui permet de mettre en place des traitements dépendant de la nullité de l'objet, sans mettre en place de structure conditionnelle.
...
Rappelons que les méthodes intermédiaires ne font effectivement aucun traitement. Si l'on doit effectuer de nombreuses, il peut-être  plus lisible de scinder le code :
[source, java]
----
